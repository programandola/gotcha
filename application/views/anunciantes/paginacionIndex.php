			...
			<ul id="ulRangosPaginacion" >
				...
				<li>
					...
					if($this->paginador->configPaginador["paginaActual"] >= 2){
						?>
						<!-- boton primero-->
						<a class="btn btn-primary fileupload-exists" title="Primero" href="<?php echo $this->varsPaginacion["url"]?>"><<</a>
						<!-- boton anterior-->
					...
					for($i=0; $i< count($rangos); $i++){
						if($rangos[$i]==$this->paginador->configPaginador["paginaActual"]){
							...
							?>
							<a class="btn btn-primary fileupload-exists" title="Pagina <? echo $rangos[$i]?>" href="<?php $this->varsPaginacion["url"]?>?pag=<?php echo $rangos[$i]?>"><?php echo $rangos[$i];?></a>
							<?php
						}
					}
						...
						if($this->paginador->configPaginador["paginas"] > 2){
							...
							if($this->paginador->configPaginador["paginas"] > $this->paginador->configPaginador["paginaActual"] ){
								?>
								<a class="btn btn-primary fileupload-exists" title="Siguiente" href="<?php $this->varsPaginacion["url"]?>?pag=<? echo $this->paginador->configPaginador["paginaActual"]+1; ?>">></a>
								<!-- boton ultimo-->
								<a class="btn btn-primary fileupload-exists" title="Ultimo" href="<?php $this->varsPaginacion["url"]?>?pag=<? echo $this->paginador->configPaginador["paginas"]; ?>">>></a>
							<?
							}
						}
						?>
